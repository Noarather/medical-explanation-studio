...
| Docling（仅历史版本） | MIT。[项目许可证](https://github.com/docling-project/docling/blob/main/LICENSE)；2.2.0 起不再捆绑。 |
| RapidOCR（仅历史版本） | Apache-2.0。[上游项目](https://github.com/RapidAI/RapidOCR)；2.2.0 起不再捆绑。 |
...
仓库仅包含应用源码、配置占位符、合成测试材料和历史模型哈希清单，不包含依赖二进制、模型权重或历史安装包。2.2.0 起使用云端 OCR，不再下载或捆绑本地 AI 模型；历史下载工具／清单不属于当前默认构建流程。云服务另受服务供应商条款约束。
