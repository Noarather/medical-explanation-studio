...
这是一个独立的 Windows 桌面程序（当前版本 2.2.3，Vue 3 + Qt WebEngine）。它为用户添加的医学教材 PDF 建立检索库，为 JSON/Excel 题目生成带教材证据的解析，支持自动校验与异常人工复核，再导出标准 JSON/XLSX 或增量文件。程序不会连接或修改其他题库网站。

## 2.2.3 更新

“题目管理”的“整批生成／重新生成解析”会将所选导入批次全部题目纳入任务，包括导入时已附带旧解析、已生成和已审核的题目。预览会显示已有解析和已审核数量；确认后旧结果由新结果替换，并重新进入待审核。旧解析不进入教材检索或生成提示。

## 2.2.2 更新

在“题目管理”选择导入批次 → “整批首次生成解析” → 确认数量与费用 → 选择可用教材。跨分页提交尚未处理且没有解析的题目，保留已有解析与审核结果；右上角“任务”查看进度，“审核工作台”查看结果。

## 2.2.1 更新

教材库新增“高级 OCR 复核”：指定单个 PDF 物理页，比较原图、当前 OCR 和 `qwen3.8-max` / `qwen3.7-plus`。明确确认后才调用收费接口，本机保存历史，不自动改写索引或切换整本教材的模型。详见 [云端 OCR](docs/CLOUD_OCR.md)。
