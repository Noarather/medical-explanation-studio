...
这是一个独立的 Windows 桌面程序（当前版本 2.2.1，Vue 3 + Qt WebEngine）。它为用户添加的医学教材 PDF 建立检索库，为 JSON/Excel 题目生成带教材证据的解析，支持自动校验与异常人工复核，再导出标准 JSON/XLSX 或增量文件。程序不会连接或修改其他题库网站。
